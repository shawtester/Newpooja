...
import { Heart, ShoppingCart, UserCircle2, Menu, X } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white bg-opacity-65 backdrop-blur-2xl py-3 px-4 md:py-4 md:px-16 border-b flex items-center justify-between">
      ...
      {/* Mobile menu */}
      <div
        className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'} absolute top-16 left-0 right-0 bg-white p-4 border-b`}
      >
        {menuList?.map((item) => (
          <Link href={item?.link} key={item.name}>
            <button className="w-full text-sm px-4 py-2 rounded-lg hover:bg-gray-50">
              {item?.name}
            </button>
          </Link>
        ))}
